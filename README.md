...
| `sim.py` | **Unfinished, and now measurably so.** A 6502 core that runs a cartridge's own code and traps its audio writes, so a player is its own authority on its format. Boots cartridges, reaches their main loops, and follows MARIA's display-list interrupts -- a walk verified against MAME byte for byte. `--compare` scores it against a known-good capture, which is the only thing that makes it trustworthy, and today it scores 6.3% on Ballblazer and 0.6% on Midnight Mutants: both run without crashing, play correct notes, and then stop advancing. Groundwork, not a tool yet. |
