...
| `sim.py` | A 6502 core that runs a cartridge's own code and traps its audio writes, so a player is its own authority on its format. It follows MARIA's display interrupts, charges for its DMA, and implements POKEY's random generator; it agrees with MAME instruction for instruction over the first 427,399 instructions after a cartridge takes control. Run like for like, it reproduces **94.8%** of forty seconds of Midnight Mutants' music with timing at 1.00x and reaching the end of the capture. A cartridge that *generates* its music from POKEY entropy rather than playing a score cannot be scored this way at all -- judge that one by whether it plays. |
