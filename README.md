...
| `sim.py` | **A TIA tool.** A 6502 core that runs a cartridge's own code and traps its audio writes, so a player is its own authority on its format. It follows MARIA's display interrupts and agrees with MAME instruction for instruction over the first 427,399 instructions after a cartridge takes control. Scored against like-for-like captures it reproduces Ikari Warriors at 99.4%, Midnight Mutants at 94.8% and Dark Chambers at 78.4%, each with timing at 1.00x; Choplifter and Donkey Kong fail, in both cases with a voice that never gets written. The POKEY path is NOT validated -- see the module docstring. |
